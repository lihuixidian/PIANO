...
void QNet::BuildNet()
{
    ...
    if (cfg::reg_hidden > 0)
    {
        ...
        h2_weight->value.SetRandN(0, 0.01);
	...
	w_n2l->value.SetRandN(0, 0.01);
	p_node_conv->value.SetRandN(0, 0.01);
	h1_weight->value.SetRandN(0, 0.01);
    fg.AddParam(w_n2l);
                           ...
                           const int* actions)
{
    ...
    {
        auto& g = g_list[idxes[i]]; //对每一个图进行处理
        int counter;
    ...
    node_feat.Reshape({(size_t)node_cnt, (size_t)2});
    node_feat.Fill(1.0);
	...
	{                
        ...
        int t = 0;
        for (int j = 0; j < g->num_nodes; ++j)   // 这里是对每个图的节点进行嵌入吗？？？
        {
            if (idx_map[j] < 0)
                continue;
            idx_map[j] = t;
